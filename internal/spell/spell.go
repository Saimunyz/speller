...
func (s *Spell) Lookup(input string, opts ...LookupOption) (SuggestionList, error) {
	...
	consideredDeletes := make(map[uint32]struct{})

	// Keep track of the suggestions we've already considered
	consideredSuggestions := make(map[uint32]struct{})
	h := getStringHash(input)
	consideredSuggestions[h] = struct{}{}
...
func addKey(hash map[uint32]struct{}, key string) bool {
	h := getStringHash(key)

	if _, exists := hash[h]; exists {
		return false
	}
